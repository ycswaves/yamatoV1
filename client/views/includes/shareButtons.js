...
Template.shareButtons.rendered = function(){
	...
	$('body').off('click','#renrenBtn').on('click','#renrenBtn',function(){
		CommonHelper.popupWindow('http://widget.renren.com/dialog/share?resourceUrl='+Router.current().originalUrl, '分享至新浪微博', 200, 200);
	})
	//分享到Facebook
	$('body').off('click','#facebookBtn').on('click','#facebookBtn',function(){
		CommonHelper.popupWindow('https://www.facebook.com/sharer/sharer.php?u='+Router.current().originalUrl, '分享至新浪微博', 600, 300);
	})
}
