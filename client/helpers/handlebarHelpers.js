...
Handlebars.registerHelper('getDistances', function(thisProperty){
  var multiAddress = Session.get('multiAddress')
    ...
    return Directions.find({fromAddr:thisProperty.address}).fetch();
});

Handlebars.registerHelper('duration', function(response) {
  if (response) {
...
Handlebars.registerHelper('distance', function(response) {
  if (response) {
    var leg = response.data.routes[0].legs[0];
